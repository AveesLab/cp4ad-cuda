...
int main(void) {
	...
	// 1차원 배열 생성 해당 부분도 수정
	uint8_t Input[(width + 2 * padding) * (height + 2 * padding)]; // input_width * output_width
	uint8_t Output[width_out * height_out];
    ...
    conv_gpu(Input, Output, width_in, height_in, width_out, height_out, in_channel, out_channel, padding, stride, kernel_size);
